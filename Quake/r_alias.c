...
=============
*/
void GL_DrawAliasFrame_GLSL (aliashdr_t *paliashdr, lerpdata_t lerpdata, gltexture_t *tx, gltexture_t *fb, unsigned int img_tex)
{
	float	blend;
...
// set uniforms
	...
	GL_Uniform1iFunc (fullbrightTexLoc, 0);
	GL_Uniform1iFunc (useFullbrightTexLoc, 0);
	GL_Uniform1fFunc (useOverbrightLoc, 0);
...
// r_alias_program will be 0.
	...
	{
		/*if (e->model->imgTex) {
			printf("found img texture\n");
		}*/
		//printf("%d\n", e->model->imgTex);
		GL_DrawAliasFrame_GLSL (paliashdr, lerpdata, tx, fb, e->model->imgTex);
		//printf("drawing name: %s\n", strcat("textures/", e->model->imgTex));
	}
	else if (overbright)
	{
