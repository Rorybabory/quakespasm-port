...
qmodel_t *Mod_ForName (const char *name, qboolean crash)
{
	...
	int foundDot = 0;
	for (int i = 6; i < MAX_QPATH; i++) {
		if (mod->name[i] == '.') { foundDot = 1; }
		if (foundDot == 1)
			mod->baseName[i - 6] = '\0';
		else
			mod->baseName[i - 6] = mod->name[i];
	}
	char tempArr[MAX_QPATH - 6];
	strcpy(tempArr, mod->baseName);
	snprintf(mod->baseName, sizeof(mod->baseName), "id1/textures/%s.png", tempArr);
	printf("load model with name %s\n", mod->baseName);
...
void Mod_LoadAliasModel (qmodel_t *mod, void *buffer)
{
	...
	glGenTextures(1, &mod->imgTex);
	glBindTexture(GL_TEXTURE_2D, mod->imgTex); // all upcoming GL_TEXTURE_2D operations now have effect on this texture object


	// load image, create texture and generate mipmaps
	int width, height, nrChannels;
	unsigned char* data = stbi_load(mod->baseName, &width, &height, &nrChannels, 0);
	if (data)
	{
